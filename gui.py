...
class Gui:
    ...
    def startGame(self):
        ...
        scoreSurface = pygame.font.SysFont(SCORE_LABEL_FONT, 50).render('Score : ', True, (0,0,0))
    ...
    def draw(self,window, matrix, cells, score, over):

        # Background and Score label
        window.fill(GRID_COLOR)
        window.blit(score[1][0], score[1][1]) # write 'score' on window
    # Score
        scoreSurface = pygame.font.SysFont(SCORE_LABEL_FONT, 50).render(str(score[0]), True, (0,0,0))
        scoreRect = scoreSurface.get_rect()
        scoreRect.top = 25
